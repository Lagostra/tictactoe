...
class ComputerPlayer:
    ...
    def get_score(self, board, maximize = True):
        ...
        best_score = self.get_score(possible_moves[0][0], not maximize)[0]
        best_move = 0
        for i in range(1, len(possible_moves)):
            move_score = self.get_score(possible_moves[i], not maximize)[0]
    ...
    def get_possible_moves(self, board):
        ...
        for y in range(len(board)):
            for x in range(len(board[y])):
                if not board[y][x]:
                    board_copy = [[y for y in x] for x in board]
